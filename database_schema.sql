-- CuraSoft Clinic Database Schema
-- Execute this script in Supabase SQL Editor to create all required tables
-- Note: Skip tables that already exist (like patients, dentists) and only create missing ones

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create missing tables only (skip patients, dentists if they exist)
-- Suppliers table
...
-- Add contactPerson column if it doesn't exist (for backward compatibility)
DO $$
BEGIN
...
-- Enable Row Level Security (RLS) - skip existing tables
...
-- Create RLS policies (users can only access their own data) - skip existing tables

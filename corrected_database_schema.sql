-- CuraSoft Clinic Database Schema - CORRECTED VERSION
-- Complete redesign based on TypeScript interfaces analysis
...
CREATE TABLE IF NOT EXISTS patients (
    ...
    dentalChart JSONB NOT NULL DEFAULT '{}',
    images TEXT[] DEFAULT '{}',
...
-- For admin operations, we'll use a service role or disable RLS temporarily during setup
-- These policies are commented out to avoid circular dependencies
-- CREATE POLICY "Admins can view all user_profiles" ON user_profiles FOR SELECT USING (true);
-- CREATE POLICY "Admins can insert user_profiles" ON user_profiles FOR INSERT WITH CHECK (true);
-- CREATE POLICY "Admins can update user_profiles" ON user_profiles FOR UPDATE USING (true);
-- CREATE POLICY "Admins can delete user_profiles" ON user_profiles FOR DELETE USING (true);

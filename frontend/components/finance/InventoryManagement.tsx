...
const AddEditInventoryItemModal: React.FC<{
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormData({ ...formData, [name]: (name === 'currentStock' || name === 'unitCost' || name === 'minStockLevel') ? parseFloat(value) : value });
    };
                                ...
                                <li key={item.id} className="flex flex-col sm:flex-row justify-between sm:items-center bg-white p-3 rounded-md shadow-sm gap-2">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-slate-500">
                                            {t('inventory.supplier')}: {supplier?.name || t('common.na')} | {t('inventory.currentStock')}: {item.currentStock}
                                        </p>
                                        {item.expiryDate && (
                                            <p className="text-xs text-slate-500">
                                    ...
                                    <div className="flex flex-col items-end sm:items-center gap-2 flex-shrink-0">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="font-semibold text-slate-700">{currencyFormatter.format(item.unitCost)}</span>
                                            <button

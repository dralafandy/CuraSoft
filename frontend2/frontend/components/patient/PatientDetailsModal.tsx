...
import { Patient, DentalChartData, Payment, NotificationType, PatientDetailTab, TreatmentRecord, ToothStatus } from '../../types';
                ...
                <main className="p-6 overflow-y-auto flex-1 bg-neutral">
                    {activeTab === 'details' && (
                        <div className="bg-white p-4 rounded-lg shadow-sm space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div><strong>{t('patientDetails.dob')}:</strong> {patient.dob ? dateFormatter.format(new Date(patient.dob)) : 'No date'}</div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {patient.images.map((image, index) => (
                                            <div key={index} className="relative">
                                                <img src={image} alt={`Patient image ${index + 1}`} className="w-full h-32 object-cover rounded-lg shadow-sm" />
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    {activeTab === 'treatments' && (
                        <div className="bg-white p-4 rounded-lg shadow-sm">
                            ...
                            <TreatmentRecordList patient={patient} clinicData={clinicData} onUpdateTreatmentRecord={(patientId, record) => {}} />
                        </div>
                    )}

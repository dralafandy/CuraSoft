...
export interface FinancialSummary {
  ...
  totalRevenue: number; // Clinic's share from payments only
...
export function useFinancialCalculations(
  ...
  return useMemo(() => {
    ...
    // Revenue calculations - clinic's share from payments only
    const clinicRevenue = filteredPayments.reduce((sum, payment) => sum + payment.clinicShare, 0);
    ...
    // Net profit = clinic revenue - operating expenses (doctor payments not subtracted)
    const netProfit = clinicRevenue - operatingExpenses;

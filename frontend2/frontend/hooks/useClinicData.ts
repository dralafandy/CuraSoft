...
import {
    ...
    LabCase, Payment, SupplierInvoice, ExpenseCategory, NotificationType, DoctorPayment
} from '../types';
...
export const useClinicData = (): ClinicData => {
    ...
    const fetchData = useCallback(async () => {
        ...
        const tables = [
            ...
            'payments', 'supplier_invoices', 'doctor_payments'
        ];
        ...
        const [
            ...
            paymentsRes, supplierInvoicesRes, doctorPaymentsRes
        ] = results;
        ...
        if (treatmentRecordsRes.data) {
            ...
            const formattedTreatmentRecords = treatmentRecordsRes.data.map((record: any) => ({
                ...
                totalTreatmentCost: record.total_treatment_cost, // total_treatment_cost -> totalTreatmentCost
    ...
    const addTreatmentRecord = async (patientId: string, record: Omit<TreatmentRecord, 'id' | 'patientId'>) => {
        ...
        const { dentistId, treatmentDate, treatmentDefinitionId, notes, inventoryItemsUsed, totalTreatmentCost, doctorShare, clinicShare, affectedTeeth } = record;
        const supabaseData = {
            patient_id: patientId,
            dentist_id: dentistId,
            ...
            total_treatment_cost: Number(totalTreatmentCost) || 0,
        ...
        if (error) {
            ...
            const newTreatmentRecords = newData.map((tr: any) => ({
                ...
                totalTreatmentCost: tr.total_treatment_cost,
    ...
    const updateTreatmentRecord = async (patientId: string, record: TreatmentRecord) => {
        ...
        // Map TreatmentRecord object to snake_case for Supabase
        const { id, patientId: pid, dentistId, treatmentDate, treatmentDefinitionId, notes, inventoryItemsUsed, totalTreatmentCost, doctorShare, clinicShare, affectedTeeth } = record;
        const supabaseData = {
            patient_id: pid,
            dentist_id: dentistId,
            treatment_date: treatmentDate,
        ...
        if (payment.treatmentRecordId) {
            ...
            if (treatmentRecord) {
                ...
                if (treatmentDefinition) {
                    // Calculate shares based on payment amount, not total treatment cost
                    clinicShare = payment.amount * (treatmentDefinition.clinicPercentage / 100);
                    doctorShare = payment.amount * (treatmentDefinition.doctorPercentage / 100);
                }
            }
        }
